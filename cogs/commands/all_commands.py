        ...
        try:
            ...
            embed = Embed(title="**была использована команда `echo`**", 
                description=f"Использовал {ctx.user.mention}\n"
                            ...
                            f"Текст:\n{message}\n\n",  
                            color=0x000000)
            embed.set_footer(text="Всё обсуждение данного нарушения должно происходить через личные сообщения для сохранения анонимности")
    ...
    async def about(self, ctx):
        ...
        try:
            embed = Embed(
                ...
                f"Создан: {creator.mention}, специально для osu!#russian"
            )
      
            await ctx.response.send_message(embed=embed, ephemeral=True)

            embed_log = Embed(title="**была использована команда `about`**", 
                description=f"использовал {ctx.user.mention}\nв канале {ctx.channel.mention}",  color=0x000000)
